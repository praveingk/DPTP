...
update_kernel_time(struct dptp_data_ts *dptp_data)
{
	...
	printf("Adjusted Kernel time by %ld ns, delta=%ld ns\n", nsec, delta);
...
static void send_dptp_request (struct dptp_data_ts *dptp_data, struct rte_mempool *mbuf_pool) {
...
void run_dptp (struct dptp_data_ts *dptp_data, struct rte_mempool *mbuf_pool) {
	unsigned nb_rx;
	struct rte_mbuf *pkt;
	bool fop_received; 
	...
	printf("DPTP - port %d\n", dptp_data->portid);
	printf("---------------------------------------\n");
	send_dptp_request(dptp_data, mbuf_pool);
...
__rte_noreturn void start_dptp(uint16_t ports, struct rte_mempool *mbuf_pool) {
	uint16_t portid;
	...
	while (1) {
		...
		for (portid = 0; portid < ports; portid++) {
			run_dptp(&dptp_data[portid], mbuf_pool);
			usleep(1000);
		}
